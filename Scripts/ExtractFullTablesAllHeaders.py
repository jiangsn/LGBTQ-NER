...
def extractFromTable(fname, paperIdx, tableIdx):
    ...
    df_value = pd.DataFrame(data=df_data, columns = ['paperIdx', 'tableIdx', 'rowIdx', 'colIdx',
...
df_table = df_table.sort_values(['paperIdx', 'tableIdx']).reset_index(drop=True)
df_table.to_csv(outPath+'TableFullRecordsFlatten.csv', encoding='utf-8', index=False)
print('Done!')
...
# df_table = pd.read_csv(outPath+'TableFullRecordsFlatten.csv', encoding='utf-8')
df_table
...
# pd.set_option('display.max_rows', 300)
select_rows = df_table['sc_ch_semantic_0'].str.contains(r"\b(Race|race|Racial|racial|Multirace|ethnicity|Ethnicity)\b")
...
# df_table[select_rows].reset_index(drop=True)
df_table.loc[select_rows, 'sc_ch_semantic_0'] = 'Time'
...
df_table_upd = pd.merge(df_table, df_race_cleaned[['paperIdx','tableIdx','rowIdx',
            'SC_level_0_clean']], on=['paperIdx','tableIdx','rowIdx'], 
            how='left').fillna('')

# %% 
df_table_upd.rename(columns={'sc_ch_semantic_0':'semantic_0'}, inplace=True)
df_table_upd.rename(columns={'SC_level_0_clean':'semantic_0_clean'}, inplace=True)
...
df_table_upd2.to_csv(outPath+'TableFullRecordsFlattenUpd.csv', encoding='utf-8', index=False)
...
# %%
location_list = []
for index, row in df_table_upd2.iterrows():
    if index % 100 == 0:
        print(index)
    txt = row['semantic_0']
    tokens = nlp(txt)
...
df_table_upd2.loc[df_location['index'],'semantic_0'] = 'Location'
...
df_table.to_csv(outPath + 'TableFullRecordsFlattenUpd.csv', encoding='utf-8', index=False)
...
#
#
df_race_table_list = df_full_upd[df_full_upd['semantic_0']=='Race/Ethnicity'][['paperIdx','tableIdx']].drop_duplicates()
df_race_only_tables = pd.merge(df_full_upd, df_race_table_list, on=['paperIdx','tableIdx'],how='inner').fillna('')
df_race_only_tables.to_csv(outPath + 'RaceOnlyTables20210922.csv', encoding='utf-8', index=False)
df_race_only_tables

# %% ########################### Pre-fill sample size info ################################
df_race_only_tables = pd.read_csv(outPath + 'RaceOnlyTables20210922.csv', encoding='utf-8').fillna('')
...
# %%  save the new table
df_race_only_tables.to_csv(outPath + 'RaceOnlyTablesPrefill.csv', encoding='utf-8', index=False)
...
# %% load a sentence representation learning model
#
#################################################################

a = 'Require minimum pack size for OTPs'
b = 'Require the posting of quit line information'
# a = df_sc_ch_unique.loc[2001, 'clean0']
# b = df_sc_ch_unique.loc[2002, 'clean0']

print(a,'\n',b)

search_doc = nlp_sentence(a)
main_doc = nlp_sentence(b)
...
print(stringRoot(a), stringRoot(b))


# %% 
# check race/ethnic terms in subjectColumn that were missed in sc_ch_semantic_0
#
race_words = ['Hispanic', 'hispanic', 'Caucasian', 'caucasian',
        'African', 'african', 'American', 'american',  
        'Race', 'race', 'races', 'ethnicity', 'Ethnicity', 'Asian', 'asian',
...
df_table.to_csv(outPath + 'FullTablesFirstColumn.csv', encoding='utf-8', index=False)
