...
outPath = './TobaccoControl/tableData/'
...
def extractFromTable(fname, pop_type):
    ...
    if len(pop_data) > 0:
        pdata = [[fname, pop_type, '; '.join(pop_data), '; '.join(pop_notes)]]
        df_value = pd.DataFrame(data=pdata, columns =['table', 'type', 'value', 'notes/details'])
    else:
        df_value = []
...
if True:
    for i, fname in enumerate(os.listdir(dataPath)):
        # if fname == 'TC.2015.62.144905.csv':
        #     print(fname)
        if i < 0: 
            break
        if (i%100)==0:
...
for index, row in pop_df.iterrows():
    ...
    for item in items:
        if not (item=='' or any_in(cnot,item) or item.split(' ')[0].isnumeric()):
            ...
            # append to list
            values.append(item_Clr) # for overall grouping
            value.append(item_Clr)
    # cnt = Counter(value)
    paperID = '.'.join(row['table'].split('.')[:3])
    popResults.append([paperID, ';'.join(value)])
    
df_popResults = pd.DataFrame(data=popResults, columns =['paperID', 'ethnicity'])
df_popResults.to_csv(outPath + 'Results.Ethnicity.byPaper.csv')
df_popResults # = df_popResults[df_popResults['ethnicity']!='']
...
# %%
a = 'White/non-Hispanic\xa0NH'
b = a.replace('/', ' ').replace('\xa0',' ')
b
# %%
