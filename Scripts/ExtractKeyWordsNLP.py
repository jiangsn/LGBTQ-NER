...
paperData = './TobaccoControl/paperData_w_figtbl.csv'
outPath = './TobaccoControl/NLP_results/'

df_paper = pd.read_csv(paperData, index_col=0, encoding='utf-8').fillna('')
df_paper.head(10)
...
# %%
# open Ethnicity and Age data to fill the current table
df_ethnic = pd.read_csv(outPath + 'Results.Ethnicity.byPaper.csv', index_col=0, encoding='utf-8')
df_ethnic.head()

# %%
df_age = pd.read_csv(outPath + 'Results.Age.Group.csv', index_col=0, encoding='utf-8')
df_age.columns = ['age', 'paperID']
df_age.head()
...
for index, row in df_paper.iterrows():
    ...
    ### get population data for this paper
    paperID = '.'.join(['TC', str(row['Year']), str(row['FirstPage'])])
    ethnicData = df_ethnic[df_ethnic['paperID']==paperID]['ethnicity'].to_list()
    if ethnicData:
        lol = [str(cell).split(';') for cell in ethnicData]
        items = [item for sublist in lol for item in sublist]
        cnt = Counter(items)
        keys = [key for key, _ in cnt.most_common()]
        ethnicData = ';'.join(keys)
    else:
        ethnicData = ''
    ageData = df_age[df_age['paperID']==paperID]['age'].to_list()
    if ageData:
        lol = [cell.split(';') for cell in ageData]
        items = [item for sublist in lol for item in sublist]
        cnt = Counter(items)
        keys = [key for key, _ in cnt.most_common()]
        ageData = ';'.join(keys)
    ...
    numTbls = row['Num of Tables']
    tblCaps = ''
    tblLinks = ''
    if numTbls>0:
        CapTxt = row['Table Captions'].split('<\>')
        linkTxt = row['Table Links'].split(' ')
        for j in range(0,min(numTbls,int((len(CapTxt)-1)/2+0.5))):
            tblCaps += CapTxt[j*2+1] + ' ' + CapTxt[j*2+2].strip() + '\n'
            tblLinks += linkTxt[j] + '\n'
        tblCaps = tblCaps.strip('\n')
        tblLinks = tblLinks.strip('\n')
    else:
        tblCaps = ''
        tblLinks = ''
            
    print(tblCaps, '\n', tblLinks)
    lines.append([row['Conference'], row['Title'], row['Link'], tblCaps, tblLinks, 
                row['Abstract'], row['Highlights'], bertTopic, myabstract, 
                myhighlight, locs_txt, ethnicData, ageData, 
                row['Year'], row['FirstPage']])

df_paper_loc = pd.DataFrame(lines, columns=['Journal', 'PaperTitle', 'PaperLink', 'TableCaps',
                                'TableLinks', 'original_abstract', 'original_highlights', 
                                '3LineTopic', '100_word_abstract', '100_word_highlight', 
                                'Location', 'Ethnicity', 'Age', 'Year', 'FirstPage'])
df_paper_loc.to_csv(outPath + 'TobaccoPapersKeySummary.csv', encoding='utf-8')
df_paper_loc.head()
